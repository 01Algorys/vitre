...
const TOTAL            = String(showcaseProjects.length).padStart(2, "0");
...
function ShowcaseItem({
  ...
  return (
    <div ref={ref} className="relative min-h-screen flex items-center overflow-hidden">
      ...
      {/* Fullbleed background image with parallax */}
      ...
      {/* Content */}
      <motion.div
        className={`relative z-10 w-full max-w-7xl mx-auto px-6 md:px-10 lg:px-16 flex ${
      ...
      >
        ...
        >
          ...
          <motion.h2
            variants={fadeUp}
            className="font-display text-[clamp(2.5rem,7vw,6.5rem)] leading-[0.88] text-white mb-7"
          >
            {project.title}
          </motion.h2>

          {/* Description */}
          <motion.p variants={fadeUp} className="text-[#aaa] text-base leading-relaxed mb-10">
            {project.description}
          </motion.p>
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap gap-2 mb-12">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="text-[9px] tracking-[0.2em] uppercase border border-white/12 px-3 py-1 text-[#888] rounded-full"
              >
                {tag}
              </span>
            ))}
          ...
          <motion.a
            ...
            data-cursor-hover
          >
            <span className="border-b border-white/30 group-hover:border-white pb-px transition-colors duration-300">
              View Project
            </span>
            <motion.span
    ...
    <section className="relative bg-[#0a0a0a]">
      ...
      <div className="section-padding max-w-7xl mx-auto">
        ...
        >
          ...
          >
            Immersive<br />
            <em className="text-gradient not-italic">Showcase</em>
          </motion.h2>
        </motion.div>
      </div>
