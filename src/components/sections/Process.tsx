...
import { processSteps as defaultSteps } from "@/lib/data";
import { fadeUp, staggerContainer, viewportConfig } from "@/lib/animations";
import { Plus, Minus } from "lucide-react";

interface Props { steps?: ProcessStep[] }

export default function Process({ steps = defaultSteps }: Props) {
    ...
    <section id="process" className="relative bg-[#111111] section-padding overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
            ...
            return (
              ...
              >
                <button
                  ...
                  data-cursor-hover
                >
                  <span className={`font-display text-5xl leading-none flex-shrink-0 transition-colors duration-500 ${isOpen ? "text-[#d4af37]/60" : "text-white/10 group-hover:text-white/20"}`}>
                    {step.number}
                  </span>
                  <span className={`flex-1 text-xl font-light tracking-wide transition-colors duration-300 ${isOpen ? "text-[#d4af37]" : "text-white group-hover:text-[#e5e5e5]"}`}>
                    {step.title}
                  </span>
                  <motion.div
                    className={`w-8 h-8 rounded-full border flex items-center justify-center flex-shrink-0 transition-colors duration-300 ${isOpen ? "border-[#d4af37]/40 text-[#d4af37]" : "border-white/10 text-[#555] group-hover:border-white/30 group-hover:text-[#999]"}`}
                  >
        ...
        >
          ...
          <span className="text-[10px] tracking-[0.45em] uppercase text-[#444]">From concept to delivery</span>
          <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#d4af37]/30 to-transparent" />
        </motion.div>
      </div>
    </section>
